...
params ["_Position","_MortarType","_Sound","_SoundOn","_TravelTime","_isFlare"];
Private ["_Height","_FallSpeed", "_Pos"];

if (_TravelTime > 2) then {	sleep (_TravelTime - 2) };

_Pos = CreateVehicle ["Land_HelipadEmpty_F", [_Position select 0, _Position select 1, ((_Position select 2) + 50)], [], 0, "CAN_COLLIDE"];
